...
void LaserProcessingClass::intensityCalibration(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in){
    for(int i=0;i<pc_in->points.size();i++){
        //std::cout <<  " old  intensity  = "<<  pc_in->points[i].intensity << std::endl;
        pc_in->points[i].intensity = pc_in->points[i].intensity /255;
        ...
        //std::cout <<  " new  intensity  = "<<  pc_in->points[i].intensity << std::endl;
    }
...
void LaserProcessingClass::featureExtraction(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_corner, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_surf, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_rest){
    ...
    for (int i = 0; i < (int) pc_in->points.size(); i++)
    {
        ...
        current_angle = current_angle + 180/57.2957;
     //std::cout <<  "angle = " << angle << std::endl;
        ...
        //    std::cout <<  "last_angle = " << last_angle << std::endl;
        ...
        //point filtering
        if (angle > 15 || angle < -24.33 || scanID>N_SCANS || scanID<0)
            continue;
        //distance
        ...
        if(distance < 0.5 && pc_in->points[i].z > -1.0)
            continue;
